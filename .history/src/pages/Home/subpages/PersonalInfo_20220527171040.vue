<template>
  <div>
    ...
    <el-dialog v-model="dialogForCharge" title="账户充值">
      ...
      <template #footer>
          <span class="dialog-footer">
              <el-button @click="onCharge" type="warning">确认充值</el-button>
          </span>
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="dialogForWithdraw">
      ...
      <template #footer>
          <span class="dialog-footer">
              <el-button @click="onWithdraw" type="warning">确认提现</el-button>
          </span>
      </template>
    </el-dialog>
  </div>
...
import {charge, withdraw} from '../../../api/subpages/PersonalInfo'

type FormInstance = InstanceType<typeof ElForm>
const chargeCash=ref(0)
const withdrawCash=ref(0)
const balance=ref(0)
...
async function onCharge(){
  let ans=await charge(chargeCash.value)
  if(ans=='success'){
    ElMessage.success('充值失败')
    dialogForCharge.value=false
  }else{
...
async function onWithdraw(){
  let ans=await withdraw(chargeCash.value)
  if(ans=='success'){
    ElMessage.success('提现失败')
    dialogForWithdraw.value=false
  }else{
